...
const WorkspaceAvatar: React.FC<WorkspaceAvatarProps> = ({ name, size = "large" }) => {
...
export default function WorkspaceDetailPage(): React.ReactElement {
  ...
  const [activeRetailerTab, setActiveRetailerTab] = useState<string>('');
  const [isActionsOpen, setIsActionsOpen] = useState<boolean>(false);
  const actionsRef = useRef<HTMLDivElement>(null);
  const dispatch = useDispatch<AppDispatch>();
  const { workspaceDetail, detailStatus: loading, detailError: error } = useSelector((state: RootState) => state.workspaceView);
  ...
  const handleEdit = (): void => console.log(`Editing workspace ${workspaceDetail?.id}`);
  const handleDuplicate = (): void => { 
    console.log(`Duplicating workspace ${workspaceDetail?.id}`); 
    setIsActionsOpen(false); 
  };
  const handleArchive = (): void => { 
    console.log(`Archiving workspace ${workspaceDetail?.id}`); 
    setIsActionsOpen(false); 
  };
  const handleDelete = (): void => { 
    console.log(`Deleting workspace ${workspaceDetail?.id}`); 
    setIsActionsOpen(false); 
  };

  // Function to organize workspace data by retailers
  const getRetailerData = (): RetailerData[] => {
    ...
    workspaceDetail.settings.forEach(setting => {
      if (setting.retailer?.retailer_name) {
        ...
        if (setting.brand) {
          retailer.categories.get(categoryName)!.brands.add(setting.brand);
        }
      }
    });
  ...
  const mainTabs: Array<{ id: TabType; label: string; icon: React.ReactElement }> = [
