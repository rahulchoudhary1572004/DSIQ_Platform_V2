...
export default function ModifyWorkspacePage(): React.ReactElement {
  ...
  const availableRetailers: AvailableRetailer[] = [
  ...
  const removeSetting = (settingId: number): void => {
    setSettings(settings.filter(s => s.id !== settingId));
    setHasChanges(true);
  };

  const updateSetting = (settingId: number, field: keyof WorkspaceSetting, value: any): void => {
