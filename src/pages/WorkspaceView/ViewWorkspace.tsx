...
interface WorkspaceSetting {
  id: number;
  retailer: WorkspaceRetailer | null;
...
interface WorkspaceCardProps {
  ...
  onArchiveToggle: (id: string | number, isArchived: boolean) => void;
  onViewDetails: (name: string, id: string | number) => void;
  isArchived: boolean;
  isLoading: boolean;
}
...
type SortBy = "name" | "retailer" | "dateCreated" | "dateModified";
type SortOrder = "asc" | "desc";
type GroupBy = "none" | "category" | "retailer";

const Tooltip: React.FC<TooltipProps> = ({ content, position = "top", children, delay = 200 }) => {
  ...
  const calculatePosition = (): void => {
    ...
    const positions: Record<PositionType, [number, number]> = {
  ...
  const transforms: Record<PositionType, string> = {
    top: "translateX(-50%)",
  ...
  const arrowStyles: Record<PositionType, React.CSSProperties> = {
    top: {
      top: "100%",
      left: "50%",
...
const NoResultsState: React.FC<NoResultsStateProps> = ({ searchQuery, onClear }) => (
...
export default function WorkspacesPage(): React.ReactElement {
  ...
  const workspacesToDisplay: Workspace[] = showArchived
  ...
  const groupedWorkspaces = ((): GroupedWorkspaces => {
    if (groupBy === "category") {
      return sortedWorkspaces.reduce<GroupedWorkspaces>((groups, workspace) => {
        const categories = workspace.settings
          ?.map((setting) => setting?.category?.name)
          .filter(Boolean) as string[] || ["Uncategorized"];
      ...
      return sortedWorkspaces.reduce<GroupedWorkspaces>((groups, workspace) => {
        const retailers = workspace.settings
          ...
          .filter(Boolean) as string[] || ["No Retailer"];
  ...
  const handleViewDetails = (name: string, id: string | number): void =>
  ...
  const handleCreateWorkspace = (): void => navigate("/workspaceCreate");

  const handleArchiveToggle = async (id: string | number, currentArchiveStatus: boolean): Promise<void> => {
    try {
      setIsLoading(true);
      await dispatch(
