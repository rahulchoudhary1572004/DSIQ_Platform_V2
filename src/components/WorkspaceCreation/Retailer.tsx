...
interface RetailersByCountry {
  [country: string]: Retailer[];
}
...
export default function Retailer({ 
...
}: RetailerProps): React.ReactElement {
  const [retailerSearch, setRetailerSearch] = useState<string>("");
  const [retailersByCountry, setRetailersByCountry] = useState<RetailersByCountry>({});
  ...
  useEffect(() => {
    const fetchData = async (): Promise<void> => {
      ...
      if (result.success && result.data) {
        const grouped = result.data.reduce<RetailersByCountry>((acc, retailer) => {
        ...
        const sortedGrouped = Object.keys(grouped)
          .sort()
          .reduce<RetailersByCountry>((acc, country) => {
        ...
        setError(result.message || "Failed to fetch retailers");
      }
      setLoading(false);
    };
  ...
  const handleRetailerChange = (retailerId: string): void => {
  ...
  const handleCountryToggle = (country: string): void => {
  ...
  const removeRetailerTag = (retailerId: string): void => {
  ...
  const shouldExpandCountry = (country: string): boolean => {
  ...
  const getCountryCodeForRetailer = (retailerId: string): string => {
