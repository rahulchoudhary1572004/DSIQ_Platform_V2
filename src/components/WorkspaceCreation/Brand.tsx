...
interface BrandProps {
  ...
  setSelectedBrands: React.Dispatch<React.SetStateAction<SelectedBrands>>;
  retailers: Retailer[];
  categoriesByRetailer: CategoriesByRetailer;
}
...
const cache: { [key: string]: BrandsByCategory } = {};
...
export default function Brand({
  ...
  const [brandSearches, setBrandSearches] = useState<BrandSearches>({});
  const [brandsByCategory, setBrandsByCategory] = useState<BrandsByCategory>({});
  ...
  useEffect(() => {
    const fetchBrands = async (): Promise<void> => {
      ...
      const result: ApiResult = await fetchBrandsByCategories(selectedCategoriesFlat);
  ...
  const handleBrandChange = (categoryId: string, brand: string): void => {
  ...
  const removeBrandTag = (categoryId: string, brand: string): void => {
    setSelectedBrands((prev) => {
      const categoryBrands = prev[categoryId] || [];
      const newBrands = { ...prev };
      newBrands[categoryId] = categoryBrands.filter((b) => b !== brand);
  ...
  const hasIncompleteBrands = (categoryId: string): boolean => {
    return (
  ...
  const getCategoryName = (categoryId: string, retailerId: string): string => {
