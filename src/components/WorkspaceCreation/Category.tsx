...
interface CategoryProps {
  ...
  retailers: Retailer[];
  setCategoriesByRetailer: React.Dispatch<React.SetStateAction<CategoriesByRetailer>>;
}

interface ApiResult {
  success: boolean;
  data?: CategoriesByRetailer;
  message?: string;
}
...
export default function Category({
  ...
  const [expandedRetailers, setExpandedRetailers] = useState<ExpandedState>({});
  const [categoryInputs, setCategoryInputs] = useState<CategoryInputs>({});
  const [categorySearches, setCategorySearches] = useState<CategorySearches>({});
  const [categoriesByRetailer, setLocalCategoriesByRetailer] = useState<CategoriesByRetailer>({});
  ...
  const debouncedSetCategorySearches = useCallback(
    debounce((retailerId: string, value: string) => {
  ...
  const handleSearchInputChange = (retailerId: string, value: string): void => {
  ...
  const categoryMatchesSearch = (name: string, data: CategoryData | null, searchTerm: string): boolean => {
  ...
  const hasMatchingDescendant = (categories: CategoryHierarchy, searchTerm: string): boolean => {
    if (searchTerm === "") return true;
  ...
  const autoExpandMatchingCategories = useCallback(
    (retailerId: string, searchTerm: string): void => {
      const hierarchy = categoriesByRetailer[retailerId]?.hierarchy || {};
      const pathsToExpand = new Set<string>();

      const findMatchingPaths = (categories: CategoryHierarchy, currentPath: string[] = []): void => {
  ...
  useEffect(() => {
    const fetchCategories = async (): Promise<void> => {
      ...
      if (selectedRetailers.length > 0) {
        const result: ApiResult = await fetchCategoriesByRetailers(selectedRetailers);
  ...
  const handleCategoryChange = (retailerId: string, categoryId: string): void => {
  ...
  const removeCategoryTag = (retailerId: string, categoryId: string): void => {
    setSelectedCategories((prev) => {
      const retailerCategories = prev[retailerId] || [];
      const newCategories = { ...prev };
      newCategories[retailerId] = retailerCategories.filter((id) => id !== categoryId);
  ...
  const toggleCategoryExpansion = (path: string): void => {
    setExpandedCategories((prev) => ({
      ...prev,
      [path]: !prev[path],
    }));
  ...
  const hasIncompleteCategories = (retailerId: string): boolean => {
    return (
  ...
  const filteredCategories = useMemo(() => {
    ...
    const filterCategories = (categories: CategoryHierarchy, parentPath: string[] = []): void => {
      Object.entries(categories).forEach(([name, { children, data }]) => {
