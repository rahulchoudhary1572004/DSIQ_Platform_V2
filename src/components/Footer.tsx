...
const Footer: React.FC<FooterProps> = ({ isLoggedIn = false }) => (
  <footer className="py-6 text-center text-sm text-gray-500">
    © 2025 DSIQ, Inc. {isLoggedIn ? '• Logged in' : ''}
  </footer>
);
...
const Footer: React.FC<FooterProps> = ({ isLoggedIn }) => {
  ...
  const navigationLinks: string[] = ['Home', 'Features', 'Pricing', 'About', 'Contact'];
    ...
    return (
      <footer className="w-full bg-gray-800 text-gray-400 py-2 shadow-inner">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center">
          <p className="text-sm">© {currentYear} DSIQ Inc. All rights reserved.</p>
    ...
    <footer className="relative bg-slate-950 text-white pt-8 mt-6 border-slate-800">
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
          ...
          <div>
            ...
            <ul className="text-sm text-slate-400 space-y-3">
              <li className="flex items-center gap-2">
                <Mail size={16} /> contact@dsiq.tech
              </li>
              <li className="flex items-center gap-2">
                <Phone size={16} /> +1 (555) 123-4567
              </li>
              <li className="flex items-center gap-2">
                <MapPin size={16} /> Tech Hub, CA
              </li>
            </ul>
          </div>
        ...
        <div className="mt-10 bg-gradient-to-r from-blue-800/20 to-cyan-800/20 backdrop-blur-md p-3 rounded-xl border border-slate-700">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Stay in the loop with the latest from DSIQ.
              </p>
            </div>
          ...
          {subscribed && (
            <p className="text-green-400 mt-2 text-sm">Thanks for subscribing!</p>
          )}
        </div>
        ...
        <div className="mt-12 py-4 flex flex-col sm:flex-row justify-between items-center text-xs text-slate-500 border-t border-slate-800">
          <p>© {currentYear} DSIQ. All rights reserved.</p>
